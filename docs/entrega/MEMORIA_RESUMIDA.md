...
Esta memoria resumida presenta una versión sintética y defendible del proyecto **Escape de la Mazmorra**. La memoria ampliada completa se conserva en `docs/MEMORIA.md` y `docs/MEMORIA.pdf`, donde se desarrollan con más detalle las decisiones técnicas, la evolución del trabajo, los problemas encontrados y la crítica final.
...
La interfaz comenzó como una idea de terminal y evolucionó hasta una aplicación JavaFX con varias pantallas. Antes de implementarla se preparó un boceto ASCII en `docs/BOCETO_JAVAFX.md`, que definía zonas funcionales: matriz de cueva, estado del jugador, inventario, acciones y log.
